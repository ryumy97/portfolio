...
import {
  attachStaggerReveal,
  type StaggerRevealController,
} from "./staggerRevealModifier";
...
const SPLAT_REVEAL_DELAY_MS = 500;
const SPLAT_REVEAL_DURATION_MS = 3000;
const SPLAT_REVEAL_GROW_MS = 350;
...
export class GaussianViewer {
  ...
  private splatRevealStartTime: number | null = null;
  private staggerReveal: StaggerRevealController | null = null;
  ...
  private animate = (): void => {
    ...
    this.updateSplatReveal();
    this.renderer.render(this.scene, this.camera);
  };

  private updateSplatReveal(): void {
    if (this.splatRevealStartTime === null || !this.staggerReveal) return;

    this.staggerReveal.tick();

    const clockTime = this.spark.clock.getElapsedTime();
    if (this.staggerReveal.isComplete(clockTime)) {
      this.finishSplatReveal();
    }
  }

  private startSplatReveal(): void {
    if (!this.splatMesh) return;

    this.staggerReveal = attachStaggerReveal(this.splatMesh, {
      delayMs: SPLAT_REVEAL_DELAY_MS,
      staggerMs: SPLAT_REVEAL_DURATION_MS,
      growMs: SPLAT_REVEAL_GROW_MS,
      camera: this.camera,
    });
    this.staggerReveal.start(this.spark.clock.getElapsedTime());
    this.splatRevealStartTime = performance.now();
    this.splatMesh.visible = true;
  }

  private finishSplatReveal(): void {
    this.staggerReveal?.detach();
    this.staggerReveal = null;
    this.splatRevealStartTime = null;
  }

  private resetSplatReveal(): void {
    this.finishSplatReveal();
  }
